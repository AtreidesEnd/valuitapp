...
export default class ValtableRow extends Component {
  ...
  renderMenu() {
    const {headerid, type, triggerValModal} = this.props;
    let children;
    if (type === 'folder') {
      children = [
        <li key={headerid + 'newfolder'} className="mdl-menu__item app-menu-item"
          onClick={() => triggerValModal({type: 'NEW_FOLDER', context: headerid})}>New Folder</li>,
        <li key={headerid + 'newdriver'} className="mdl-menu__item app-menu-item"
          onClick={() => triggerValModal({type: 'NEW_DRIVER', context: headerid})}>New Driver</li>
      ];
    } else {
      children = [
        <li key={headerid + 'newdriver'} className="mdl-menu__item app-menu-item"
